        ...
        if xmsbt.endswith('.xmsbt'):  # Ignore files that don't end with extension .xmsbt
            if xmsbt in editedFiles or not directoryMode:  # If directory, ignore any files that are not in boths dirs
                ...
                if edited: #Only write to file if there's anything changed
                    ...
                    final.write(Path("out").joinpath(xmsbt), encoding="utf-16")
                    with open(Path("out").joinpath(xmsbt),'r', encoding="utf-16") as xFile:
                        outFile = xFile.read()
                    for m in re.finditer("µ#x[0-9A-F]+;", outFile):
                        outFile = outFile[:m.start()] + "&" + outFile[m.start() + 1:]
                    with open(Path("out").joinpath(xmsbt),'w', encoding="utf-16") as xFile:
                        xFile.write(outFile)
                    print("Done!")