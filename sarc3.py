...
# sarc3.py is a slightly modified version of ObsidianX's original sarc.py,
# made to work on more recent versions of Python and improve UX

# You can find the original at https://github.com/ObsidianX/3dstools

# TODO: this is still a bit broken for whatever reason, try to fix it
...
class Sarc:
    ...
    def __init__(self, filename, directory="", compressed=False, verbose=False, extract=False, debug=False, little_endian=True,
    ...
    def add(self, path):
        if os.path.isdir(path):
            for path, dirs, files in os.walk(path):
                self._add_path(path.replace("\\", "/"), dirs, files)
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('-D', '--directory', metavar='DIRECTORY', help='directory to extract the files to', default="")
    ...
    sarc = Sarc(args.archive, directory=args.directory, compressed=args.zlib, verbose=args.verbose, debug=args.debug, extract=args.extract,
